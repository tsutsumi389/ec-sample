...
#
# 未登録の商品は仕様欄が SKU とカテゴリだけになる（壊れはしない）。
PRODUCT_SPECS: dict[str, list[tuple[str, str]]] = {
    "wireless-earphone": [
...
def seed_data(db: Session) -> None:
    ...
    for item in SEED_PRODUCTS:
        ...
        product = Product(
            ...
            specs=[
                ProductSpec(label=label, value=value, sort_order=index)
                for index, (label, value) in enumerate(
                    PRODUCT_SPECS.get(item["slug"], [])
                )
            ],
        )
        db.add(product)
